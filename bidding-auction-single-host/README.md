...
### 3. Setup the Environment on the Test Machine
...
### 4. Launch the B&A Local Testing Application
...
### 5. Launch the Local TVS and Public Key Endpoint
...
### 6. Launch the B&A Stacks

The demo utilizes two B&A stacks, `stack_a` and `stack_b`, located in the `./ba_hats_stack/` folder. These stacks contain BFE (Bidding Frontend), Bidding, Auction, and SFE (Seller Frontend) services.

**Important:** Launch each stack in a separate Tmux window with the following specific order: "bidding" -> "auction" -> "BFE" -> "SFE". Use the `launch-ba.sh` script with each `.prototext` file.

For example, to launch the auction service for `stack_a`:

```bash
./launch-ba.sh stack_a/auction.prototext
```

Wait for the log message `Server listening on 0.0.0.0` to indicate successful service startup.
...
- [~~~~MacOS~~~~](https://support.apple.com/guide/keychain-access/add-certificates-to-a-keychain-kyca2431/mac)p
...
Transfer the generated `demo` folder from the build machine to the test machine (the SEV-SNP device).
